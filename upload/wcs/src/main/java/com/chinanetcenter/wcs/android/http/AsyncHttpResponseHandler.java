 ...
 * {@link com.chinanetcenter.wcs.android.http.AsyncHttpClient}. The
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
	...
	private static HashMap<String, String> sIpList = new HashMap<String, String>();
	 ...
	 */
	public void onProgress(int bytesWritten, int totalSize) {
		WCSLogUtil.d(String.format("Progress %d from %d (%d%%)", bytesWritten, totalSize, (totalSize > 0) ? (bytesWritten / totalSize) * 100 : -1));
	}
	...
	public void onRetry(int retryNo) {
		Log.d("CNCLog", String.format("Request retry no. %d", retryNo));
	}
	...
	public void sendResponseMessage(HttpResponse response) throws IOException {
		...
		if (!Thread.currentThread().isInterrupted()) {
			...
			StringBuffer headersStringBuffer = new StringBuffer("headers : \n");
			for (Header header : allHeaders) {
				headersStringBuffer.append(header.getName() + " : " + header.getValue());
				headersStringBuffer.append("\n");
			}
			long contentLength = responseBody.length;
			String responseString = new String(responseBody);
			String destIp = getIpByUrl(requestURI.toString());
			String responseIdentifier = String.format("*** time : %s,\r\n *** destIp : %s,\r\n *** uri : %s,\r\n *** status : %s,\r\n *** length : %s\r\n", timestamp,
					destIp, requestURI.toString(), statusCode, contentLength);
			LogRecorder.getInstance().dumpLog(responseIdentifier);
			LogRecorder.getInstance().dumpLog(headersStringBuffer.toString() + "\n");
			LogRecorder.getInstance().dumpLog("*** response : " + responseString + "\n");

			// additional cancellation check as getResponseData() can take
			// non-zero time to process
			if (!Thread.currentThread().isInterrupted()) {
				if (statusCode >= 300) {
					sendFailureMessage(statusCode, response.getAllHeaders(), responseBody,
							new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()));
				} else {
					sendSuccessMessage(statusCode, response.getAllHeaders(), responseBody);
				}
			}
		}
	}
